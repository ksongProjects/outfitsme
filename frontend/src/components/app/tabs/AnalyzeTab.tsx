...
import {
  ...
  useSettingsContext,
  useWardrobeContext,
} from "@/components/app/DashboardContext";
...
export default function AnalyzeTab() {
  ...
  >({});
  const [expandedAnalysisKeys, setExpandedAnalysisKeys] = useState<
  ...
  const availableImageModel = (modelOptions || []).find(
    (model) => model.supports_image && model.available,
  );
  const firstUnavailableImageModel = (modelOptions || []).find(
    (model) => model.supports_image && !model.available,
  );
  const imageGenerationEnabled = Boolean(
    settingsForm?.enable_outfit_image_generation,
  );
  ...
  const queueSummaryText =
    activeAnalysisCount > 1
      ...
      : "";
  const statusLabel =
    stageLabelByKey[jobStatus?.status || ""] || jobStatus?.status || "Pending";
  const stageLabel =
    stageLabelByKey[progress?.stage || ""] || progress?.stage || "";
  const analysisInProgress =
    loading ||
    ["queued", "processing_started", "submitting"].includes(
      jobStatus?.status || "",
    );
  ...
  useEffect(() => {
    ...
    const latestKey = String(
      latestEntry.job_id || latestEntry.result.photo_id || "latest-analysis",
    );
  ...
  const handleCropPointerDown = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    const resizeHandle = (event.target as HTMLElement | null)?.dataset
      ?.handle as "nw" | "ne" | "sw" | "se" | undefined;
  ...
  const handleCropPointerMove = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (interaction.mode === "resize" && interaction.startCrop) {
      ...
      const startBottom =
        interaction.startCrop.y + interaction.startCrop.height;
  ...
  const handleGenerateOutfitsMe = async (
    ...
    if (!imageGenerationEnabled) {
      toast.error(
        "OutfitsMe image generation is off. Enable it in Settings > Features.",
      );
      return;
    }
    if (!profilePhotoUrl) {
      toast.error(
        "Profile photo is required for OutfitsMe. Upload one in Settings > Profile.",
      );
      return;
    }
      ...
      <div className="tab-header">
        <div className="tab-header-title">
          ...
          <p className="tab-header-subtext">
            Upload a look, crop the key area, and turn it into reusable wardrobe
            data.
          </p>
        </div>
      </div>
      ...
      <div className="analysis-layout">
        <Card as="section" className="c-surface c-surface--stack">
          <div className="c-section-head o-stack o-stack--tight">
            ...
            <p className="subtext">
              Best results come from a clear full-body or torso-focused outfit
              photo.
            </p>
          </div>
          ...
          {modelOptions.length > 0 && !availableImageModel ? (
            <div className="settings-notice">
              <p>
                <strong>Analysis unavailable:</strong> The managed AI service is
                not currently available.
              </p>
              <p className="subtext">
                {firstUnavailableImageModel?.unavailable_reason ||
                  "No image-capable model is currently available."}
              </p>
            </div>
          ) : null}
          ...
          <div className="quota-pill" role="status" aria-live="polite">
            {limitsLoading ? (
              ...
              <span>
                Trial: {usedToday}/{dailyLimit} used today ({remainingToday}{" "}
                left), {trialDaysRemaining} day
          ...
          {jobStatus ? (
            <div className="info-stack">
              ...
              {progressCounts &&
              typeof progressCounts.total_items === "number" &&
              progressCounts.total_items > 0 ? (
                <p className="subtext">
                  Item images: {progressCounts.processed_items || 0}/
                  {progressCounts.total_items || 0} processed,{" "}
                  {progressCounts.generated_items || 0} generated,{" "}
                  {progressCounts.failed_items || 0} failed
                </p>
              ) : null}
              {typeof progressCurrentItem?.index === "number" ? (
                <p className="subtext">
                  Current item {progressCurrentItem.index}:{" "}
                  {formatItemLabel(progressCurrentItem)}
                </p>
              ) : null}
            </div>
          ) : null}
          ...
          {activeAnalysisCount > 0 ? (
            <p className="subtext">
              Active analysis jobs: <strong>{activeAnalysisCount}</strong> /{" "}
              {maxConcurrentAnalysisJobs}
            </p>
          ) : null}
          ...
          {previewUrl ? (
            <>
              ...
              <p className="subtext">
                Drag on the preview to select a rectangular area. Only the
                selected crop is sent to analysis.
              </p>
            </>
          ) : null}
        ...
        <Card as="section" className="c-surface c-surface--stack">
          <div className="o-split o-split--start o-split--stack-sm analysis-results-head">
            <div className="c-section-head o-stack o-stack--tight">
              <h3>Results</h3>
              <p className="subtext">
                Completed analyses stay here for this session, with each outfit
                grouped into an expandable result card.
              </p>
            </div>
          ...
          {shouldCollapseResults && !resultsExpanded ? (
            <p className="subtext">
              Multiple jobs are queued. Expand this panel to review the latest
              completed analysis while the queue finishes.
            </p>
          ) : null}
          ...
          {!shouldCollapseResults || resultsExpanded ? (
            <div id="analysis-results-body" className="o-stack">
              {analysisInProgress ? (
                <div
                  className="loading-panel o-cluster o-cluster--start"
                  role="status"
                  aria-live="polite"
                >
                  <Spinner className="loading-spinner-lg" aria-hidden="true" />
                  <div>
                    <p className="loading-panel-title">Analysis in progress</p>
                    <p className="subtext">
                      {progress?.message ||
                        "Your photo is being analyzed. Results will appear here automatically."}
                    </p>
                  </div>
                </div>
              ) : null}
              ...
              {!completedAnalyses.length && !loading ? (
                <p className="subtext">
                  Analyze a photo to view detected outfits and reusable item
                  breakdowns.
                </p>
              ) : null}
                ...
                <div className="analysis-results-accordion">
                  {completedAnalyses.map((entry, analysisIndex) => {
                    ...
                    const outfits = result.outfits?.length
                      ? result.outfits
                      : [{ style: result.style, items: result.items }];
                    ...
                    const isExpanded =
                      expandedAnalysisKeys[analysisKey] ?? analysisIndex === 0;
                    const itemCount = outfits.reduce(
                      (total, outfit) => total + (outfit.items?.length || 0),
                      0,
                    );
                    const summaryStyle =
                      outfits.find((outfit) =>
                    ...
                    return (
                      <Card
                        as="article"
                        key={analysisKey}
                        className="c-surface c-surface--stack"
                      >
                        <button
                          type="button"
                        ...
                        >
                          <span className="analysis-result-group-summary">
                            <span className="result-label">
                              Analysis{" "}
                              {completedAnalyses.length - analysisIndex}
                            </span>
                            <span>
                        ...
                        {isExpanded ? (
                          <div
                            id={`${analysisKey}-body`}
                            className="analysis-result-group-body"
                          >
                            {outfits.map((outfit, outfitIndex) => {
                              const previewKey = buildOutfitsMePreviewKey(
                                result.photo_id,
                                outfitIndex,
                              );
                              const outfitPreviewUrl =
                                ...
                                <>
                                  <ul className="o-list" key={previewKey}>
                                    {(outfit.items || []).map(
                                      (item, itemIndex) => (
                                        <li
                                          key={`${analysisKey}-item-${outfitIndex}-${itemIndex}`}
                                  ...
                                  <div className="o-cluster o-cluster--end ">
                                    <Button
                                      type="button"
                                      variant="outline"
                                      onClick={() =>
                                        handleGenerateOutfitsMe(
                                          result.photo_id,
                                          outfitIndex,
                                        )
                                      }
