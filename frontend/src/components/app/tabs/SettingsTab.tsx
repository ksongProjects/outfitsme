    ...
    <section className="settings-layout">
        ...
        <div className="tab-header">
          <div className="tab-header-title">
            ...
            <p className="tab-header-subtext">
              Manage your profile, feature access, and the operational side of
              your AI wardrobe.
            </p>
            <p className="subtext">
              Review the <Link href="/terms">Terms of Service</Link> and{" "}
              <Link href="/privacy">Privacy Policy</Link> for deployment-ready
              legal coverage.
            </p>
          </div>
        </div>
      ...
      <div className="o-stack o-stack--section">
        <Card
          as="article"
          id="settings-profile"
          className="c-surface c-surface--stack settings-section-card"
        >
          <h2>Profile</h2>
          ...
          <ImageUploadField
            ...
            disabled={profilePhotoUploading || profileSaving}
          />
          <div className="o-cluster o-cluster--wrap o-cluster--stack-sm">
            <Button
              variant="outline"
              onClick={async () => {
                const uploaded = await uploadProfilePhoto(profilePhotoFile);
                if (uploaded) {
                  setProfilePhotoFile(null);
                }
              }}
              disabled={
                !profilePhotoFile || profilePhotoUploading || profileSaving
              }
            >
              {profilePhotoUploading
                ? "Uploading..."
          ...
          <p className="subtext">
            Access level: <strong>{userRole}</strong>
          </p>
          <Input
            id="settings-name"
            value={profileName}
          ...
          >
            <SelectTrigger id="settings-gender" className="w-full">
              <SelectValue placeholder="Select gender">
                {(value) =>
                  GENDER_LABELS[String(value || "unspecified")] ||
                  "Select gender"
                }
              </SelectValue>
            </SelectTrigger>
          ...
          <Input
            ...
            onChange={(event) =>
              setSettingsForm((prev) => ({
                ...prev,
                profile_age: event.target.value,
              }))
            }
            placeholder="Age"
          />
        ...
        >
          <h2>Features</h2>
          <p className="subtext">
            Turn advanced capabilities on only when they fit the experience you
            want to offer users.
          </p>
                ...
                <Label>
                  <Checkbox
                    ...
                    onCheckedChange={(value) =>
                      setSettingsForm((prev) => ({
                        ...prev,
                        enable_outfit_image_generation: Boolean(value),
                      }))
                    }
                  />
                  <strong>Outfit image generation</strong>
                </Label>
              </div>
              <p className="subtext">
                Generate item thumbnails and composed outfit visuals using your
                ...
                <Label>
                  <Checkbox
                    ...
                    onCheckedChange={(value) =>
                      setSettingsForm((prev) => ({
                        ...prev,
                        enable_accessory_analysis: Boolean(value),
                      }))
                    }
                  />
                  <strong>Accessory analysis</strong>
                </Label>
              </div>
              <p className="subtext">
                Include bags, jewelry, and other accessories during photo
                analysis.
              </p>
            </div>
          </div>
            ...
            <div>
              <div className="flex items-center gap-2">
                <Label>
                  ...
                  />
                  <strong>Online store search (Coming soon)</strong>
                </Label>
              </div>
              <p className="subtext">
                Retail search is temporarily disabled while we finish the
                storefront integration.
              </p>
            </div>
          </div>
          <div className="o-cluster o-cluster--wrap o-cluster--stack-sm">
            <Button onClick={saveFeatureSettings} disabled={featureSaving}>
              {featureSaving ? "Saving..." : "Save feature settings"}
            </Button>
          </div>
        </Card>
        ...
        >
          ...
          {costSummaryLoading ? (
            ...
            <>
              <p className="subtext">
                Month start:{" "}
                {costSummary.month_start_utc
                  ? new Date(costSummary.month_start_utc).toLocaleString()
                  : "-"}
              </p>
              <ul className="o-list o-list--split">
                <li>
                  Analysis runs:{" "}
                  <strong>{costSummary.analysis_runs ?? 0}</strong>
                </li>
                <li>
                  Composed outfits created:{" "}
                  <strong>
                    {costSummary.composed_outfits_created ??
                      costSummary.custom_outfit_generations ??
                      0}
                  </strong>
                </li>
                <li>
                  Try-on generations:{" "}
                  <strong>{costSummary.try_on_generations ?? 0}</strong>
                </li>
              </ul>
              ...
              <ul className="o-list o-list--split">
                <li>
                  Analysis:{" "}
                  <strong>
                    ${costSummary.estimated_costs_usd?.analysis ?? 0}
                  </strong>
                </li>
                <li>
                  ...
                  <strong>
                    $
                    {costSummary.estimated_costs_usd?.outfit_image_generation ??
                      0}
                  </strong>
                </li>
                <li>
                  ...
                  <strong>
                    $
                    {costSummary.estimated_costs_usd?.item_image_generation ??
                      0}
                  </strong>
                </li>
                <li>
                  Total:{" "}
                  <strong>
                    ${costSummary.estimated_costs_usd?.total ?? 0}
                  </strong>
                </li>
              </ul>
              ...
              <ul className="o-list o-list--split">
                <li>
                  Input tokens:{" "}
                  <strong>
                    {costSummary.token_usage_estimate?.total?.input_tokens ?? 0}
                  </strong>
                </li>
                <li>
                  Output tokens:{" "}
                  <strong>
                    {costSummary.token_usage_estimate?.total?.output_tokens ??
                      0}
                  </strong>
                </li>
                <li>
                  Total tokens:{" "}
                  <strong>
                    {costSummary.token_usage_estimate?.total?.total_tokens ?? 0}
                  </strong>
                </li>
              </ul>
              {!costSummary.analysis_runs &&
              !(
                costSummary.composed_outfits_created ??
                costSummary.custom_outfit_generations
              ) &&
              !costSummary.try_on_generations &&
              !(costSummary.estimated_costs_usd?.total ?? 0) ? (
            ...
            <p className="subtext">
              Cost usage unavailable: {costSummaryError}
            </p>
          ) : (
            <p className="subtext">Cost usage unavailable.</p>
          )}
          <div className="o-cluster o-cluster--wrap o-cluster--stack-sm">
            <Button variant="outline" onClick={() => void refreshCosts()}>
              Refresh costs
            </Button>
          </div>
        </Card>
